...
export default function DashboardCategoriesIndex() {
  ...
  const fetchCategories = async () => {
    ...
    const response = await fetch(
      `https://api-blog-express-9dbb509347a4.herokuapp.com/api/categories?limit=5` +
  ...
  const handleDelete = async (e, slug) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://api-blog-express-9dbb509347a4.herokuapp.com/api/categories/${slug}`,
        {
          method: "DELETE",
          credentials: "include",
        }
        ...
        <div className="w-full px-3 mx-auto">
          <div className="flex flex-wrap mt-0 mx-3 *:shrink-0 *:w-full *:max-w-full *:px-3 *:mt-0 py-8">
            <div className="grow shrink-0 basis-0">
              <h1 className="font-extrabold">Dashboard Categories</h1>
            </div>
          </div>
          <div className="flex flex-wrap mt-0 mx-3 *:shrink-0 *:w-full *:max-w-full *:px-3 *:mt-0 py-8">
            <div className="grow shrink-0 basis-0">
                ...
                <nav aria-label="Page navigation example">
                  <ul className="flex items-center -space-x-px h-10 text-base">
                    ...
                    {pageNumbers.map((number) => (
                      <li key={number}>
                        <Link
                          to={`/dashboard/categories?page=${number}`}
