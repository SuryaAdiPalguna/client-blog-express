...
export default function DashboardPostsEdit() {
  ...
  useEffect(() => {
    if (bodyInputRef.current) {
      ...
      if (editorElement) {
        editorElement.editor.loadHTML(body);
      }
    }
  }, [body]);
  ...
  useEffect(() => {
    const fetchPost = async () => {
      const response = await fetch(
        "https://api-blog-express-9dbb509347a4.herokuapp.com/api/posts/" +
          slugParams,
        {
          credentials: "include",
        }
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      fetch(
        "https://api-blog-express-9dbb509347a4.herokuapp.com/api/posts/slug?title=" +
          title,
        {
          credentials: "include",
        }
  ...
  const handleSubmit = async (e, slugParams) => {
    ...
    try {
      ...
      navigate("/dashboard/posts", {
        state: { success: "Post has been updated!" },
      });
    } catch (error) {
      console.error(error);
    }
        ...
        <div className="w-full px-3 mx-auto sm:max-w-3xl md:max-w-4xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl">
          <div className="flex flex-wrap mt-0 mx-3 *:shrink-0 *:w-full *:max-w-full *:px-3 *:mt-0 py-8">
            <div className="grow shrink-0 basis-0">
              <h1 className="font-extrabold">Edit Post</h1>
            </div>
          </div>
          <div className="flex flex-wrap mt-0 mx-3 *:shrink-0 *:w-full *:max-w-full *:px-3 *:mt-0 py-8">
            <div className="grow shrink-0 basis-0">
              ...
              >
                ...
                <div className="mb-5">
                  ...
                  <input
                    ...
                    defaultValue={body}
                  />
                  <trix-editor input="body" ref={trixEditorRef}></trix-editor>
                </div>
                <button
                  type="submit"
